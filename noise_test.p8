...
function create_chunk(ch,cx,cy)
	local min_n=1000
	local max_n=-1000
	local t={}
	...
	for i=0,c_size-1 do
		...
		local f=fade[j][i]*norm
		
		local n=0
		if ch.t=="tree" then
			n=decode_tree(f)
		elseif ch.t=="water" then
			n=decode_water(f)
		elseif ch.t=="grass" then
			n=decode_grass(f)
		end
		lvl[y][x]=n
	end end
...
function decode_tree(n)
	if(n>=0.65)return s_tree
	if(n>=0.4)return s_t_grass
	if(n>=0.3)return s_grass
	return 0
end

function decode_water(n)
	if(n>=0.8)return s_d_water
	if(n>=0.5)return s_water
	if(n>=0.3)return s_grass
	return 0
end

function decode_grass(n)
	if(n>=0.5)return s_t_grass
	if(n>=0.4)return s_grass
	return 0
end
