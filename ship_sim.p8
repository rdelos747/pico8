...
function _update()
	...
	if map_srch then
		...
		if(btnp2(⬇️))idx=min(idx+1,#comps+5)
	end
	
	if (idx==3 and last_idx==2) or
				(idx==#comps+4 and last_idx==#comps+5) then
		cam_targ=128
	elseif idx==2 and last_idx==3 then
		cam_targ=0
	elseif idx==#comps+5 and last_idx==#comps+4 then
		cam_targ=256
	end
	...
	elseif idx>3 and idx<#comps+4 then
		comps[idx-3]:update()
	elseif idx==#comps+4 then
		if btnp(⬅️) then
